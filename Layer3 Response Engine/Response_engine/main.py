...
--------------------
  POST /respond
    ...
    4. LLM reasoning + HITL gate    → reasoning dict
    5. Enqueue to priority queue    (all alerts require approval)
    6. Return pending_approval

  POST /approve/{alert_id}
    1. Z3 re-verify playbook.executable_actions ⊆ dqn_actions
    2. Execute via ActionExecutor
    3. Insert into expert_demonstrations (RLHF)
    4. Return IncidentResponse

  POST /admin/reject
    1. Pop from pending queue
    2. Insert into rejected_demonstrations (RLHF negative)
    3. Return status: rejected

  POST /admin/override
    Step A: Z3 Pass 2 (is_human_reviewed=True) — HTTP 400 if denied
    Step B: LLM Pass 2 (human_reviewed=True)
    Step C: Playbook Pass 2
    Step D: Execute via ActionExecutor
    Step E: Insert into expert_demonstrations
...
# All imports at module level — not inside request handlers
...
# [IMMUNEX-PATCH] Step 1: database module for asyncpg pool lifecycle and RLHF writes
from response_engine import database as _db
...
# [IMMUNEX-PATCH] Bug 10: ts_queued stored as float (time.perf_counter()) so
# the TTL sweeper can compute elapsed age without timezone gymnastics.
_priority_queue: list[tuple[int, str, str]] = []  # heapq of (priority, timestamp_iso, alert_id)
_pending_lookup: dict[str, tuple[dict, Any, Any, dict, int, str, float]] = {}
# alert_id → (alert, decision, verification, reasoning, priority, ts_iso, arrival_monotonic)

# [IMMUNEX-PATCH] Bug 10: background TTL sweeper task handle
...
# [IMMUNEX-PATCH] Bug 10: Background TTL sweeper — runs every 60 s,
# evicts _pending_lookup entries whose arrival_monotonic is > 1 hour old.
# Prevents OOM growth if operators never process their approval queues.
async def _ttl_sweeper() -> None:
    """
    Periodically removes stale entries from _pending_lookup.

    [IMMUNEX-PATCH] Bug 10: sweeps every _TTL_SWEEP_INTERVAL_S seconds;
    drops any entry older than _TTL_MAX_AGE_S (1 hour) by comparing
    the stored arrival_monotonic against the current perf_counter.
    """
    while True:
        await asyncio.sleep(_TTL_SWEEP_INTERVAL_S)
        now = time.monotonic()
        stale = [
            aid for aid, v in _pending_lookup.items()
            if (now - v[6]) > _TTL_MAX_AGE_S    # v[6] = arrival_monotonic
        ]
        for aid in stale:
            _pending_lookup.pop(aid, None)       # [IMMUNEX-PATCH] Bug 9: safe pop
            logger.warning("pending_entry_ttl_evicted", alert_id=aid, max_age_s=_TTL_MAX_AGE_S)
        if stale:
            logger.info("ttl_sweep_complete", evicted=len(stale), remaining=len(_pending_lookup))
...
async def respond(alert_req: AlertRequest) -> IncidentResponse:
    ...
    - LLM reasoning
    - Priority queue enqueue (ALL alerts require approval)
    - Immutable audit log entry (AuditLogger)

    [IMMUNEX-PATCH] Bug 7: The LOW-risk auto-execute bypass has been removed.
    ALL alerts now go to the pending approval queue regardless of risk level,
    enforcing the "ALL actions require approval" architecture constraint.
    """
    # [IMMUNEX-PATCH] Bug 6: capture request_arrival_time at endpoint entry
    # BEFORE any async awaits, so queue-wait time cannot consume the rate window.
...
async def approve(
    ...
    [IMMUNEX-PATCH] Bug 8: enforces playbook.executable_actions ⊆ dqn_actions
    after playbook generation. Strips hallucinated LLM action IDs before
    passing to ActionExecutor.
    ...
    try:
        # Pre-execution: use a lightweight dummy to let the playbook inform
        # which actions the LLM recommends — then enforce ⊆ dqn_actions (Bug 8).
        dummy_exec = ExecutionResult(
            alert_id=alert_id, action_index=decision.action_index,
            action_name=decision.action_name, actions=decision.actions,
            action_names=decision.action_names, status="pending",
            dry_run=True, execution_time_ms=0.0, output={}, error=None,
            validation_status={}
        )
        pre_playbook = _generator.generate(alert, decision, verification, dummy_exec)

        # Bug 8: enforce playbook.executable_actions ⊆ dqn_actions
        if getattr(pre_playbook, "executable_actions", None):
            safe_executable = [
                a for a in pre_playbook.executable_actions
                if a in dqn_actions
            ]
            if safe_executable != pre_playbook.executable_actions:
                stripped = set(pre_playbook.executable_actions) - dqn_actions
                logger.warning(
                    "playbook_hallucinated_actions_stripped",
                    alert_id        = alert_id,
                    stripped_ids    = list(stripped),
                    safe_executable = safe_executable,
                )
            if safe_executable:
                decision.actions           = safe_executable
                decision.action_names      = [ACTION_NAMES.get(a, "unknown") for a in safe_executable]
                decision.action_categories = [get_action_category(a) for a in safe_executable]

        # Execute with the final (possibly playbook-refined) action set
        execution = _executor.execute(decision, alert, approval_granted=True)

        # Post-execution: regenerate playbook with the REAL execution result
        # so the audit trail reflects the actual outcome, not the dummy status.
