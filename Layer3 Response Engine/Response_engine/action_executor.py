...
class ExecutionResult:
    ...
    action_index:      int
    action_name:       str
    status:            str          # "executed" | "simulated" | "failed" | "pending_approval"
    dry_run:           bool
...
class ActionExecutor:
    ...
    def execute(
        ...
        # ── Dispatch ───────────────────────────────────────────────────────
        handler = self._dispatch.get(action_idx)
        if handler is None:
            return ExecutionResult(
                alert_id          = alert_id,
                action_index      = action_idx,
                ...
                execution_time_ms = 0.0,
                output            = {},
                error             = f"No handler registered for action_index={action_idx}",
            )
    ...
    def _handle_block_source_ip(self, alert: dict) -> dict:
        ...
        self.logger.info(
            ...
            "or Palo Alto dynamic block list"
        )
        return {"target_ip": src, "rule": "DROP", "firewall": "iptables/PaloAlto"}
...
if __name__ == "__main__":
    ...
    for idx in range(50):
        decision = ActionDecision(
            ...
            requires_approval = idx in _HIGH_IMPACT,
            confidence        = 0.9,
            severity          = "high",
        ...
        # Pass approval for high-impact so we exercise the handler, not the gate
        ...
        correct_status = result.status in {"simulated", "executed", "failed", "pending_approval"}
    ...
    gate_decision = ActionDecision(
        ...
        action_category   = get_action_category(30),
