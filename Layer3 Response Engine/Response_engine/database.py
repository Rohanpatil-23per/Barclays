...
# ── Data extraction for RL Fine-Tuning ──────────────────────────────────────

async def fetch_expert_demonstrations(limit: int = 1000) -> list[dict]:
    """
    Fetches up to `limit` expert demonstrations used for Behavioral Cloning.
    Returns a list of dicts: {"state_vector": [float...], "expert_action": int}
    Takes the first action in expert_actions if there are multiple.
    """
    pool = get_pool()
    if pool is None:
        return []
    ...
    try:
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT state_vector, expert_actions
            ...
            for row in rows:
                state_str = row["state_vector"]
                # Convert vector string "[0.1, 0.2]" to list of floats
                if isinstance(state_str, str):
                    state_vec = [float(x) for x in state_str.strip("[]").split(",")]
                else:
                    state_vec = list(state_str) # In case asyncpg casts it

                actions = row["expert_actions"]
                if actions and len(actions) > 0:
                    results.append({
                        "state_vector": state_vec,
                        "expert_action": actions[0]
                    })
        return results
    except Exception as exc:
        logger.error("fetch_expert_demonstrations_failed", error=str(exc))
        return []

async def fetch_rejected_demonstrations(limit: int = 1000) -> list[dict]:
    """
    Fetches up to `limit` rejected demonstrations used for negative sampling.
    Returns a list of dicts: {"state_vector": [float...], "rejected_action": int}
    """
    pool = get_pool()
    if pool is None:
        return []
    ...
    try:
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT state_vector, rejected_actions
                FROM rejected_demonstrations
                ORDER BY timestamp DESC
                LIMIT $1
                """,
            ...
            for row in rows:
                ...
                if isinstance(state_str, str):
                    ...
                    state_vec = list(state_str)

                actions = row["rejected_actions"]
                if actions and len(actions) > 0:
                    results.append({
                        "state_vector": state_vec,
        ...
        logger.error("fetch_rejected_demonstrations_failed", error=str(exc))
        return []
