...
Output valid JSON only. No markdown fences. No preamble. No trailing text."""
    ...
    def _fallback_report(
        ...
        return PlaybookReport(
            ...
            executable_actions    = [decision.action_index] if decision.action_index is not None else [],
    ...
    def _default_summary(alert: dict, decision: ActionDecision) -> str:
        return (
            ...
            f"The response pipeline completed with status: {getattr(decision, 'action_categories', ['unknown'])[0]}."
        )
...
if __name__ == "__main__":
    ...
    action_name       = "block_c2_domain",
    actions           = [19],
    action_names      = ["block_c2_domain"],
    validation_status = {"pre_check": "passed"},
    status            = "simulated",
    dry_run           = True,
    ...
    fenced_json = (
        ...
        '"playbook_steps": ["Step 1"], "executable_actions": [19], "compliance_notes": "RBI", '
        '"recommended_followup": ["Action 1"]}\n'
        "```"
    )
    parsed = gen_parse._parse_response(fenced_json, "test-id")
    ok_parse = parsed.get("risk_level") == "HIGH" and "playbook_steps" in parsed and parsed.get("executable_actions") == [19]
