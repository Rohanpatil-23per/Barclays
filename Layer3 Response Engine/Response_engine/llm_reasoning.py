...
async def generate_llm_reasoning(
    ...
    action_name:     str,
    z3_result:       str,
    # [IMMUNEX-PATCH] Bug 12: callers pass these booleans so the reasoning
    # dict (and downstream audit log) records exactly who pressed the button.
...
Respond ONLY in JSON with EXACT keys: "reason", "risk", "business_impact", "alternative"."""
    ...
    _fallback: dict = {
        ...
        "auto_approved":   auto_approved,    # [IMMUNEX-PATCH] Bug 12
        "human_reviewed":  human_reviewed,   # [IMMUNEX-PATCH] Bug 12
    }
